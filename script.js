...
// DATA BARANG OLEH-OLEH & KEBUTUHAN REALISTIS JOGJA
const daftarBarang = [
...
// Daftar Nama Kasir Jawa yang Khas
const daftarKasir = ["Siti", "Budi", "Nugroho", "Sri", "Eko", "Tri", "Agus", "Wati", "Yanto", "Rini"];

// FUNGSI UTAMA UNTUK MENGACAK STRUK
function generateOtomatis() {
    // 1. Pilih Toko Secara Acak dari 100 Pilihan di Atas
    ...
    // 4. Set Waktu Realtime Sekarang (WIB)
    ...
    document.getElementById("receipt-date").innerText = `${tgl}/${bln}/${thn} ${jam}:${mnt}`;

    // 5. Pilih 2 hingga 6 Jenis Barang Acak Belanjaan
    const jumlahBarang = Math.floor(Math.random() * 5) + 2; 
    let htmlBarang = "";
    let totalHarga = 0;

    // Acak urutan barang agar tidak terjadi item ganda
    const barangDiacak = [...daftarBarang].sort(() => 0.5 - Math.random());

    for (let i = 0; i < jumlahBarang; i++) {
        const barang = barangDiacak[i];
        const qty = Math.floor(Math.random() * 3) + 1; // Pembelian 1-3 pcs
...
// FUNGSI DOWNLOAD GAMBAR KE STORAGE HP
function downloadPNG() {
    const target = document.getElementById("receipt-target");
    
    // Scale diubah ke 3 agar resolusi hasil screenshot struk sangat tajam dan HD saat dikirim di WA/Telegram
    html2canvas(target, { scale: 3, backgroundColor: "#ffffff" }).then(canvas => {
        const link = document.createElement('a');
        link.download = 'Struk_JogjaBorobudur_' + Date.now() + '.png';
        link.href = canvas.toDataURL('image/png');
        link.click();
    });
}

// Menjalankan generator pertama kali saat web dibuka
